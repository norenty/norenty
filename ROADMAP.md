...
- [ ] [LOOP] **viaje.remolque_requerido en el asistente de nuevo viaje** -- el campo ya existe en el esquema pero el wizard nuevo-w todavia no lo pide. Anadir un selector opcional (tipo de remolque + rango de temperatura si es frigorifico) reutilizando guardarRequisitosRemolqueViaje.
- [ ] [DECISION] **Aviso si el remolque asignado no cumple el requisito del cliente** -- comparar viaje.remolque_requerido/temperatura_requerida_* contra el remolque acoplado de verdad y avisar si no coincide. Util pero no se construye sin que el item de arriba (pedirlo en el wizard) exista primero -- no hay nada que comparar todavia.
...
- [ ] [LOOP] **Checklist de salida antes de arrancar la ruta** -- distinto del checklist de enganche (23.C.5, que ya existe para cuando se cambia de remolque). Disparador: la PRIMERA vez que el chofer interactua con el bot para un viaje que sigue en estado planificado (mismo punto donde hoy se llama a send_next_hito por primera vez para ese viaje). Preguntas si/no tipo lista (neumaticos, luces, frenos, carga asegurada) via teclado inline, mismo patron que sello_keyboard (F13.7) y anotacion_entrega_keyboard (23.A.2) -- reutilizar el patron de pregunta+callback+ejecucion_evento, no inventar uno nuevo. Si alguna respuesta es "no", crea una incidencia real via alertar_gestor() (no una notificacion pasiva) para que el gestor lo sepa antes de que el camion arranque. Guardar como ejecucion_evento tipo checklist_salida con el detalle de que fallo.
