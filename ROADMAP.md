...
- [x] `[LOOP 19.1]` **🔴 `getInformeNomina`** — **HECHO 2026-07-25**: `viaje`/`hito` ahora se piden
...
- [x] [LOOP] **Restriccion dura de vuelta a casa** -- HECHO 2026-08-02. Decision final del usuario tras discutirlo: regla DURA por defecto ("apuntamos a un algoritmo que optimice lo que un humano no ve"), con via de excepcion explicita -- mismo patron que 18.A.2 (pendiente_aprobacion + solicitud_aprobacion, ahora con tipo 'vuelta_a_casa'). Umbral CONFIGURABLE por empresa (no fijo en codigo, pedido explicito: "depende de la situacion, costes, ingresos"), default 150 km, editable en Ajustes -> Vuelta a casa el fin de semana. 0084_vuelta_a_casa.sql: chofer.ciudad_residencia_lat/lon (geocodificadas UNA VEZ al guardar via Nominatim, reutilizando buscarDireccion -- no en cada viaje), empresa.umbral_vuelta_casa_km, solicitud_aprobacion.tipo += 'vuelta_a_casa'. calcularAvisoVueltaCasa() en createViaje: si el ultimo hito con ventana_fin de la semana cae en VIERNES y la distancia real por carretera (OSRM) a la ciudad de residencia supera el umbral, el viaje nace pendiente_aprobacion. Nunca bloquea por FALTA de dato (sin chofer, sin ciudad geocodificada, sin OSRM). 12 tests nuevos (444 en total, todos en verde), verificado en navegador.


## Flota mas grande + fichas realistas (2026-07-30)

Migracion 0079: chofer.tipo_colaboracion (empleado/autonomo -- "podemos subcontratar autonomos como choferes"), vehiculo.subtipo (lona/caja_cerrada/frigorifico/cisterna/portacontenedor) + temperatura_min/max real del frigorifico, viaje.remolque_requerido + temperatura_requerida_min/max (lo que pide el CLIENTE para ese viaje, deliberadamente separado del remolque real asignado). UI: vehiculos/[id] muestra/edita subtipo+temperatura (solo si es remolque) y ya tenia el historial de mantenimiento completo (mantenimiento_vehiculo, existia desde el Milestone 3, solo faltaba sembrar datos); choferes/[id] muestra empleado/autonomo + ciudad de residencia. Sembrados en produccion: 12 choferes nuevos (total 20, 5 por delegacion) con tractora+remolque real, subtipo variado, acoplamiento, GPS e historial de mantenimiento (tractora Y remolque) cada uno -- verificado en vivo en norenty.com.

- [x] [LOOP] **viaje.remolque_requerido en el asistente de nuevo viaje** -- HECHO 2026-08-02. Selector opcional en el paso 2 (Asignacion) del wizard, junto a "Valor de mercancia": tipo de remolque + rango de temperatura si es frigorifico, reutilizando guardarRequisitosRemolqueViaje tras crear el viaje. SUBTIPO_LABEL movido de vehiculos/[id]/page.jsx a labels.js (fuente unica, mismo patron que ESTADO_VIAJE/TIPO_DOC_LABEL) para no duplicarlo. Verificado en navegador: selector con las 5 opciones, campos de temperatura aparecen solo con "Frigorifico". 427 tests en verde.
- [x] [LOOP] **Aviso si el remolque asignado no cumple el requisito del cliente** -- HECHO 2026-08-02. `getAvisoRemolqueRequeridoViaje(viajeId)` compara viaje.remolque_requerido/temperatura_requerida_* contra el remolque REAL asignado (viaje.remolque_id -> vehiculo.subtipo/temperatura_min/max); banner naranja en viajes/[id] si no coincide (subtipo distinto, o rango de temperatura del remolque no cubre el pedido). Solo lectura -- no reasigna nada, el gestor decide. Verificado con un viaje real en dev (DEMO-004): banner mostro el mensaje correcto, luego revertido. Tambien getAvisosRemolqueRequerido() (version en bloque, toda la empresa) por si hace falta un listado en el futuro. 432 tests en verde.
- [ ] [DECISION] **Logica de nomina/facturacion para autonomos** -- hoy tipo_colaboracion es solo un dato visible; un autonomo no deberia entrar en el calculo de nomina de 23.B (el factura el, no se le paga como asalariado). Falta decidir el flujo exacto antes de construirlo.

## Checklist de salida + alerta de contradiccion de ubicacion (2026-07-30)

Pedido por el usuario, con caso real de su amigo (cambio de camiones sin avisar, detectado tarde por un WhatsApp de un camionero). Dos items relacionados, ambos con spec ya bastante cerrada:
...
- [x] [LOOP] **Alerta proactiva de contradiccion de acoplamiento** -- HECHO 2026-08-02. `backend/db/monitor_acoplamiento_contradiccion.py`, mismo patron que monitor_checkpoint_saltado.py: detecta hitos en_curso/completado con remolque_id sin acoplamiento vigente, o acoplado a OTRO chofer distinto del asignado -- crea incidencia real (tipo `acoplamiento_contradiccion`, dedup por hito_id) y avisa por Telegram a los gestores con notif_incidencias activa. Frecuencia: mismo grupo cron de 15 min que los otros monitores "en curso" (decision tomada, no era necesario preguntar -- coherencia con lo ya existente). Job anadido a .github/workflows/monitores.yml. Etiqueta en TIPO_INCIDENCIA_LABEL. Verificado con 3 casos reales contra la BD de dev (rollback, nada guardado): sin acoplamiento vigente, dedup en la segunda pasada, chofer no coincide. NOTA: como los demas monitores de este workflow, no se activa de verdad hasta rellenar los GitHub Secrets (DATABASE_URL, TELEGRAM_BOT_TOKEN) en Settings -> Secrets -- pendiente de confirmar si ya estan rellenos.
...
- [ ] [DECISIÓN] **Indicador de estado/fase del chofer en vivo, cruzando dos fuentes** (peticion 2026-08-08, durante smoke test) -- semaforo por viaje: circulando / parada / cargando / descargando / pernocta, con desviacion vs planificacion (ej. "+5 min" / "-5 min"). Doble fuente que se validan entre si: (1) inferido de `ubicacion` (pings GPS ya existentes, ver `UMBRAL_GEO_LLEGADA_M` en backend/app/bot.py como precedente de deteccion por proximidad) comparando contra coordenadas de cada hito y su ventana horaria; (2) boton explicito en el bot para que el chofer marque a mano en que fase esta. Por decidir antes de construir: que tabla/columna nueva hace falta para guardar la fase actual del viaje, radio en metros y umbral de tiempo para "esta en la ubicacion de carga/descarga", como se combinan ambas fuentes cuando discrepan (¿cual manda?, ¿se avisa al gestor de la discrepancia?), y donde se muestra en el dashboard (columna en la lista de /viajes vs. panel en el detalle del viaje).