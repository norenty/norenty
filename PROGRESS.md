...
alerta_bot_caido/alerta_integridad | 2c99751 | HECHO. verdad_observada (10.8) probada
...
no lanza. 205 vitest, ci.ps1 completo verde.
...
Sin cambios de código en este commit (solo decisiones + doc); las implementaciones siguen en
items aparte.
...
2026-07-19 | Mentoria estrategica: analisis del proyecto contra marcos de fundadores/CTOs de exito | (por commitear) | HECHO (peticion explicita del usuario, no auditoria del loop). Nuevo MENTORIA-ESTRATEGICA.md: analisis contra Thiel (Zero to One), Paul Graham/YC, Andreessen (product-market fit), Horowitz (The Hard Thing About Hard Things), Ries (Lean Startup) y el Startup Genome Report (3.200 startups), mas casos reales investigados (Convoy -- cierre 2023, tesis de "sustituir brokers humanos por tecnologia" fallida; Samsara -- exito via contratos largos + ACV alto; fallos de adopcion de TMS por sobrecarga cognitiva del usuario final). Conclusion principal: el proyecto tiene el patron numero 1 de fracaso del Startup Genome Report ("premature scaling") -- 16 fases de producto construidas muy por delante de validacion de cliente real (0 viajes reales por Telegram, 0 clientes de pago). Identificado tambien que SI encaja (el diferenciador de evidencia hash-chain es un monopolio real estilo Thiel; la disciplina "calm company" de coste ~0 EUR/mes; los gates ya existentes -- Bloque F, GATE 12, Anti-roadmap Fase 9 -- son el anticuerpo correcto, solo falta hacerlos cumplir). Anadido un GATE MAESTRO al principio de ROADMAP.md: el loop no abre fases [LOOP] de feature nueva hasta que 10.1 (smoke test real) y 12.3 (discovery real) esten cerrados. Sin ci.ps1 aplicable (documentacion, no codigo).

---

2026-07-19 | Landing publica en norenty.com (un solo dominio, sin subdominio de app) | (por commitear) | HECHO a peticion explicita del usuario: todo bajo norenty.com, sin app.norenty.com aparte. Nuevo LandingPage.jsx (dashboard/app/components/): argumento de venta directo (cadena de evidencia hash-chain, sin hardware/integracion, escalacion de alertas, aislamiento verificado), reusando el sistema de diseno ya existente (colores/tipografia de globals.css) en vez de duplicar el CSS del founding-partner-onepager.html. AuthGuard.jsx: toggle de estado local (mostrarLogin) -- sin sesion, se ve la landing primero; boton "Acceder" muestra el LoginPage de siempre en la MISMA URL/dominio, sin ruta ni redireccion nueva. Excepcion preservada: un enlace de invitacion (?invitacion=codigo) salta directo al registro sin pasar por la landing (leido via window.location en un efecto, no useSearchParams -- evita el bailout de renderizado estatico que rompio el build de la pagina de ajustes en el primer intento, corregido antes de cerrar). Verificado en navegador real: landing se ve sin sesion, boton de acceso lleva al login, invitacion salta la landing, sin errores de consola. ci.ps1 completo verde (236 backend, 418 vitest, build 22 paginas).

---

2026-07-19 | Diseno cerrado: escalera de captura de conocimiento A->B->C (bot de llamadas / voz) | (por commitear) | HECHO (diseno, NO build -- decision de CTO de no picar codigo de feature 12h antes del discovery que define la taxonomia). Sesion estrategica con el usuario: identificado que el valor maximo esta en las excepciones (camion roto, hora limite) resueltas por telefono que el sistema hoy no ve. Reencuadre central: el dato del moat es la DECISION del jefe de trafico (situacion -> decision -> resultado), no el problema del chofer -- corpus propietario que nadie mas tiene, combustible de la autonomia futura (7B.7). Escalera por severidad: Nivel 1 (nota de voz -> transcribe -> clasifica, Whisper self-host, primero a construir) / Nivel 2 (deteccion R1 -> pregunta) / Nivel 3 (voz: opcion A capturar la resolucion justo despues -recomendada, coste ~0, legal- vs B duena del puente telefonico). Correcciones tecnicas honestas registradas: un bot de Telegram NO puede hacer/grabar llamadas (Bot API); "escucha pasiva" de una llamada humano-a-humano no existe; la parte de ingenieria real no es la voz (Retell/Vapi la resuelven) sino la API de acciones ACOTADA que el agente puede tocar + la identidad por caller-ID. Costura agnostica de proveedor (no casarse con Vapi/Retell). Validado parcialmente por el gestor amigo: llamadas incluso multilingues son clave. Actualizado ROADMAP.md (bloque "DISENO DE LA ESCALERA DE CAPTURA" en Fase 11), reescrito SPECS-BOT-LLAMADAS.md entero (apuntaba al caso equivocado), y anadidas 4 preguntas al discovery (DISCOVERY-GESTOR.md 3.4c) que cierran la taxonomia antes del build. GATE: el build del Nivel 1 arranca tras el discovery (taxonomia real) + deploy + 11.5 (consentimiento). Sin ci.ps1 (documentacion).

---

2026-07-19 | Verificador de integridad para foto de gasto (verificar_gasto_foto.py) | (por commitear) | HECHO. A peticion del usuario de seguir trabajando de forma autonoma mientras el prepara el deploy -- decision explicita de NO tocar la escalera de captura de voz (gated por el discovery de manana) y en su lugar repetir el mismo audit pattern que ya dio resultado hoy (verificar_incidencia_foto.py): gasto_viaje.foto_hash_sha256 existe desde 12.1/0043 (fotos de tickets/gastos, hash calculado en el navegador antes de subir) pero, a diferencia de pod e incidencia, nunca tuvo su propio verificador. verificar_gasto_foto.py: mismo patron exacto que verificar_pod.py/verificar_incidencia_foto.py (reutiliza calcular_hash_sha256), pero bucket "documentos" (no "pods" -- gasto_viaje reutiliza el bucket de documentos legales, no el de POD/incidencia), --todos filtra a gastos con foto_url IS NOT NULL (la mayoria de gastos no tienen foto). 3 tests nuevos (mismo FakeBucket/FakeStorageCliente en memoria, nunca toca Storage real): coincide/sustituido/ausente. ci.ps1 completo verde (239 backend, 418 vitest, build 22 paginas).

---

2026-07-19 | 11.3 Nivel 1: nota de voz del chofer anclada al viaje (SIN transcribir) | (por commitear) | HECHO. Cambio de criterio explicito: el gate "esperar al discovery" se cae al saber que el gestor esta de vacaciones y la charla no tiene fecha -- bloquear indefinidamente es tan mal error como construir a ciegas. Solucion: construir el SUSTRATO que vale para cualquier taxonomia (capturar y anclar el audio) y dejar fuera solo la POLITICA que depende del gestor (niveles de urgencia, clasificacion, cuando escala a llamada). Migracion 0058_contexto_audio.sql: contexto.audio_url + audio_hash_sha256 (nullable) + 'nota_voz' en el CHECK de canal (los reservados llamada_transcrita/whatsapp se dejan intactos -- una nota de voz asincrona NO es una llamada transcrita). bot.py: handle_voice (registrado con filters.VOICE) descarga el audio, hashea ANTES de subir (misma tesis de evidencia que POD 9.8 / incidencia F14.6 / gasto 12.1), sube al bucket privado pods con ruta {empresa_id}/voz/{viaje_id}/, e inserta contexto anclado al viaje en curso; sin viaje en curso avisa y NO sube nada (evita audios huerfanos). texto guarda MARCADOR_NOTA_VOZ_SIN_TRANSCRIBIR porque contexto.texto es NOT NULL (0042) -- Whisper (gated por deploy + 11.5) lo reemplazara RETROACTIVAMENTE sobre las filas ya guardadas, asi que el corpus empieza a acumularse HOY. 2 claves i18n nuevas en los 8 idiomas. HALLAZGO: getContexto (11.2) existia desde la Fase 11 pero NO se renderizaba en ninguna parte del dashboard -- el conocimiento capturado era invisible; nuevo ContextoViajeSection.jsx (feed de contexto en la ficha del viaje, con boton Escuchar via URL firmada de 60s, mismo patron que GastosViajeSection) lo estrena. 2 tests e2e nuevos (se ancla correctamente / sin viaje no guarda nada). ci.ps1 completo verde (241 backend, 418 vitest, build 22 paginas). Migracion NO aplicada todavia (sin DATABASE_URL de produccion en esta sesion) -- se aplica en el deploy.

---

2026-07-19 | 7B.2 Nivel 2: triaje de urgencia de incidencias sobre hechos objetivos | (por commitear) | HECHO. Desbloqueado por un argumento del usuario que acepto y cambia el gate: la operativa rutinaria del sector es conocida y SE PUEDE INVESTIGAR, asi que clasificar urgencia deja de ser "una taxonomia inventada" -- lo que necesita al gestor es afinar umbrales, no definir de cero. INVESTIGACION REAL hecha antes de picar (busqueda web sobre clasificacion de incidencias en transporte por carretera): da los TIPOS habituales del sector (mecanicas/operativas/climaticas/administrativas: averias, retrasos, dano de mercancia, documentacion, paralizaciones) que YA coinciden con el enum del codigo, pero HALLAZGO HONESTO: **no existe una escala de urgencia estandar en transporte por carretera** que se pueda implementar tal cual (los resultados de "severity levels" son de gestion de incidencias de IT, no de trafico). Decision de diseno en consecuencia: NO inventar una escala subjetiva 1-5, sino DERIVAR la urgencia de hechos objetivos que el sistema ya tiene. `calcularUrgenciaIncidencia(incidencia, {hito, viaje, ahora})` pura en data.js: accidente -> alta siempre (seguridad de personas); incidencia ya escalada (escalada_en, R1 nivel 2) -> alta (nadie respondio); ventana de entrega vencida -> alta con los minutos exactos; ventana dentro de UMBRAL_VENTANA_EN_RIESGO_MIN (120, valor inicial razonable NO pactado con cliente, mismo estatus que UMBRAL_POD_TARDIO_MIN) -> media; averia distingue viaje en_curso (alta, bloquea) de planificado (media, hay margen para reasignar); rechazo_entrega/documento_invalido en ruta -> media. Devuelve `motivos` ademas del nivel: el gestor tiene derecho a saber POR QUE (mismo principio de "sugerencia explicable, nunca caja negra" que sugerirChofer/10.9b). Se CALCULA en vivo, no se persiste, a proposito: la urgencia de un retraso crece segun se acerca la ventana, un valor congelado al crear la incidencia quedaria obsoleto en minutos (por eso tampoco se ordena la lista globalmente por urgencia -- eso exigiria almacenarla; el badge es lo que hace que deje de estar todo al mismo nivel). /incidencias: el select ahora trae viaje.estado y hito.ventana_fin (sin ellos no hay hechos que evaluar) y pinta badge "Urgente"/"Atencion" con los motivos en el title. 7 tests nuevos. ci.ps1 completo verde (241 backend, 425 vitest, build 22 paginas). Verificado en navegador con sesion demo real: sin errores de consola y las filas siguen renderizando (prueba de que el join nuevo con hito es valido); ninguna incidencia demo lleva badge, que es el comportamiento CORRECTO (tipo otro/incidencia_cliente, sin escalar, sin ventana vencida) -- la logica en si queda cubierta por los tests unitarios, no afirmo haber visto el badge pintado.


---

2026-07-21 | Despliegue real: Vercel + Railway + Supabase produccion | (por commitear) | HECHO. Primer despliegue real del proyecto (D4), ejecutado en vivo con el usuario. Proyecto Supabase de produccion (norenty-prod) creado; conexion via pooler (no directa, que resuelve solo IPv6 y falla en redes sin IPv6); las 58 migraciones aplicadas de golpe, sin datos que perder (proyecto vacio). Dos gaps reales de historial de migraciones encontrados y corregidos al desplegar contra un proyecto nuevo (nunca se habian visto porque el dev original se aplico a mano por MCP, sin pasar por migrate.py desde cero): (1) 0008_rls_endurecido.sql hacia REVOKE sobre rls_auto_enable(), funcion que el propio Supabase crea en la plantilla interna de algunos proyectos pero no en todos -- ahora el REVOKE es condicional. (2) 0009_tenancy_multiempresa.sql asumia gestor.auth_user_id ya existente, pero esa columna nunca se creo en ninguna migracion (se anadio a mano en dev) -- anadida al principio de 0009 con su indice unico. Ambas ya estaban en ALLOWLIST_DRIFT_CONOCIDO del propio runner, asi que el dev existente no se rompe. Dashboard desplegado en Vercel (norenty.vercel.app), root directory dashboard/, region Frankfurt. Bot desplegado en Railway, root directory backend/, region EU West, modo webhook activado. HALLAZGO Y FIX: requirements.txt tenia python-telegram-bot[job-queue] sin el extra webhooks -- el bot crasheaba en arranque real con RuntimeError (nunca se habia visto porque en local siempre corre en modo polling). Corregido a [job-queue,webhooks]. 241 pytest verde tras el cambio. Decision de producto en paralelo: quitado el autoregistro publico de LoginPage.jsx -- el modelo es venta B2B directa, no autoservicio; ahora solo se puede entrar por invitacion (mecanismo ya existente, 6.9). Incidente de seguridad durante el despliegue: el asistente pego accidentalmente el contenido de ~/.norenty-secrets/.env en el chat en dos ocasiones -- error reconocido, protocolo corregido (lectura silenciosa, solo resultados enmascarados/booleanos de ahi en adelante). Usuario decidio posponer la rotacion de credenciales para mas tarde. Pendiente: dominio norenty.com en Cloudflare, GitHub Secrets del cron de monitores, smoke test 10.1 contra el entorno ya desplegado.

---

2026-07-21 | Fase 17 planificada: de "desplegado" a "usable por un cliente real" | (por commitear) | HECHO a peticion del usuario (hoja de ruta post-despliegue: evaluacion de estado + todo lo pospuesto + rediseno de onboarding). Evaluacion honesta: los 3 servicios estan vivos y ~666 tests verdes, pero desplegado != operativo. HALLAZGO BLOQUEANTE encontrado al planificar: NADIE puede entrar en produccion, literalmente -- la BD de prod esta vacia (0 cuentas), hoy se quito el autoregistro publico, y no existe ningun script de alta de empresa real (solo seed_demo.py, que es demo). Pasa a ser el primer item ejecutable del plan (B.1, crear_empresa_cliente.py). SEGUNDO HALLAZGO, de diseno: el scoping de Fase 15 (RLS por gestor) CHOCA de frente con el requisito de continuidad que planteo el usuario ("si alguien esta de vacaciones su trabajo debe hacerlo otra persona") -- con F15.2, los viajes de un gestor ausente son invisibles para sus companeros y el trabajo se para, justo lo contrario de lo que se necesita. Anotado como decision C.1 con 4 opciones evaluadas (delegacion temporal / scoping por equipo / scoping blando solo-UI / solo admin cubre) y recomendacion explicita del CTO: delegacion temporal, por ser el patron enterprise estandar y el cambio mas pequeno que mantiene la frontera de seguridad. Plan en 6 bloques: A seguridad inmediata (rotar las 3 credenciales expuestas, separar ficheros dev-prod, segundo bot de Telegram), B desbloquear el producto (script de alta + cuenta admin propia + smoke test 10.1), C rediseno de onboarding (la tension C.1 + invitaciones verificadas de punta a punta + gating comercial manual), D operacion real (Sentry, GitHub Secrets, plan de pago de Railway antes de que caduque el trial, regiones documentadas, rate-limit del endpoint publico, CSP a enforcing, dominio www), E bloqueado por terceros (discovery, legal, Whisper y consentimiento, playbooks), F deuda tecnica conocida (isolation.test.js, npm audit, backfill de PROGRESS, OSRM, restore drill). GATE 17 definido: no se invita a ningun cliente real hasta cerrar rotacion + smoke test + Sentry + monitores. Sin codigo en este commit, solo planificacion.


---

2026-07-22 | crear_empresa_cliente.py + tests herméticos (Bloque B.1 de Fase 17) | (por commitear) | HECHO. Sustituye al autoregistro que se quito de LoginPage.jsx: usa auth.admin.invite_user_by_email (Supabase crea la cuenta e invita al propio admin a poner su contrasena, ni el script ni nosotros la vemos), luego crea empresa + gestor (rol=admin, activo=true). Rechaza email duplicado ANTES de invitar. Si falla a mitad tras invitar, el error lo dice explicito (usuario de Auth huerfano) en vez de fallar en silencio. 3 tests nuevos con fakes en memoria. HALLAZGO Y FIX DE HIGIENE DE TESTS: al reorganizar los ficheros de secretos dev/prod esta misma sesion, TELEGRAM_BOT_TOKEN quedo vacio en el de dev -- revento 13 tests de test_bot_e2e.py con InvalidToken, porque bot.py carga el fichero de secretos personal con override=True al importarse y la suite dependia sin darse cuenta de su contenido real. Nuevo tests/conftest.py: fija dummies validos para TELEGRAM_BOT_TOKEN/SUPABASE_URL/SUPABASE_ANON_KEY y redirige Path.home() a un directorio temporal vacio para que ese load_dotenv no encuentre nada que sobreescribir -- la suite queda hermetica, no depende del fichero personal de quien la corra. 240 pytest + 4 skip, 425 vitest, build 22 paginas verde.


---

2026-07-22 | D.4: regiones reales documentadas en subprocesadores (fix de un dato desactualizado) | (por commitear) | HECHO. Vercel Frankfurt, Railway EU West (Amsterdam), Supabase eu-central-1 -- confirmadas contra los proyectos reales desplegados el 2026-07-21. HALLAZGO: PRIVACIDAD-SUBPROCESADORES.md decia que Supabase estaba en eu-west-1 (Irlanda), pero el proyecto de produccion real (norenty-prod) esta en eu-central-1 (Frankfurt) -- dato desactualizado corregido. Reflejado en los dos sitios que el propio documento exige mantener sincronizados (el .md y la pagina publica de subprocesadores). Anadida nota sobre Cloudflare: gestiona correo (MX/DKIM/SPF) y DNS del dashboard, pero el registro hacia Vercel esta en DNS only (sin proxy), asi que no trata trafico del dashboard en transito hoy. Verificado en navegador: pagina carga bien, sin errores de consola. Build 22 paginas verde.


---

2026-07-22 | Fix real: mensaje claro cuando un Telegram ya esta vinculado a otro chofer | (por commitear) | HECHO, encontrado en vivo durante el primer smoke test en dev. chofer.chat_id es UNIQUE en BD (un mismo Telegram no puede representar a dos choferes a la vez, si no get_chofer_by_chat no sabria a cual responder). Vincular un Telegram ya usado por otro chofer reventaba el UPDATE con un IntegrityError sin capturar (postgrest.exceptions.APIError, code 23505), que caia en el manejador de errores generico (manejar_error, item 8.2) y respondia "problema tecnico, intentalo en un minuto" sin explicar nada -- el usuario real no tenia forma de saber que pasaba. cmd_start ahora consulta ANTES del UPDATE si el chat_id ya esta vinculado a OTRO chofer distinto del que se esta intentando vincular, y si es asi lo dice explicito con el nombre del chofer real ("Tu Telegram ya esta vinculado a otro chofer (Nombre). Contacta con tu gestor si quieres cambiarlo"). 1 test e2e nuevo (test_e2e_start_con_telegram_ya_vinculado_a_otro_chofer_da_mensaje_claro) que reproduce el caso exacto. ci.ps1 verde (242 backend, sin cambios dashboard). Contexto del hallazgo: durante el smoke test, el chat_id de prueba ya estaba vinculado a un chofer de fixture huerfano ("Mario (chofer test)", id 00000000-0000-0000-0000-000000000100, de otra empresa de pruebas) -- se libero ese registro a mano (dato, no codigo) para poder continuar, y de paso se encontro y arreglo el bug de fondo.


---

2026-07-22 | Rediseno real del asistente de nuevo viaje (hallazgos en vivo del smoke test) | (por commitear) | HECHO. Buscadores con texto (Buscador.jsx reutilizable) para chofer, vehiculo, remolque y cliente, filtrando por matricula mas marca mas modelo, con foco select-all para evitar el parpadeo de cursor en medio del texto viejo. Buscador manual de chofer entre TODOS ademas del top-5 sugerido. Referencia autogenerada (getReferenciaSugerida, correlativo VJ-0001), prellenada pero editable. Primera parada fija Recogida, ultima fija Entrega, no editables; intermedias con tipo libre, insertadas siempre antes de la ultima, con botones de mover. Corregido un bug real de paso: nuevoHito() ponia entrega en las DOS paradas iniciales por defecto. Autocompletado de direcciones ya usadas (getDireccionesGuardadas, reutiliza hito.direccion, lat y lon historicos, sin geocodificador externo). Conflicto de vehiculo, remolque o chofer ahora BLOQUEA si el otro viaje esta en_curso ahora mismo (antes siempre era aviso); sigue en aviso si el conflicto es solo con un viaje planificado a futuro. Confirmado antes de tocar codigo que YA EXISTIAN: rutas recurrentes (pagina Plantillas del menu) y separar cola de viajes sin asignar de la asignacion (crear sin chofer, asignar despues desde la ficha). 10 tests nuevos o actualizados. ci.ps1 verde (428 vitest). Verificado en navegador con sesion real de dev sin errores de consola. Pendiente: comprobacion de viabilidad tiempo-distancia entre paradas, mas diseno necesario, proxima iteracion.


---

2026-07-22 | Comprobacion de viabilidad tiempo-distancia entre paradas | (por commitear) | HECHO. Cierra el pendiente del rediseno del asistente de viaje. calcularAvisosViabilidad(hitos, velocidadKmh) pura en data.js: reutiliza Haversine por FACTOR_SINUOSIDAD_FALLBACK (mismo criterio que kmAproxViaje) y calcularEtaConParadas (el modelo 561 y 2006 ya existente, con descansos obligatorios) en vez de inventar un umbral nuevo. Solo evalua pares de paradas consecutivas con ambas coordenadas y ambas ventanas -- sin ventana no hay nada objetivo que comprobar. Es un aviso visible, no bloquea el guardado, mismo principio de sugerencia del resto del proyecto. 4 tests nuevos. ci.ps1 verde (432 vitest). Verificado en navegador con sesion real: Madrid a Burdeos en 1h muestra el aviso correcto (720 km, 21.4 h necesarias, 1 h disponible, no parece viable), sin errores de consola.
