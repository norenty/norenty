...
| `SUPABASE_SERVICE_ROLE_KEY` | Clave de servidor (SALTA RLS) | Supabase → Project Settings → API. **Sin ella, el bot en vivo no puede escribir con RLS activo** — es la `[DECISIÓN D1]` crítica del roadmap. Nunca la pegues en un chat; ábrela tú mismo en el panel y pégala en el archivo. |
...
| `DATABASE_URL` | Connection string de Postgres (para `migrate.py` y backups) | Supabase → Project Settings → Database → Connection string → URI. Pendiente `[DECISIÓN D2]` — sin ella, aplica migraciones vía MCP/panel en vez de `migrate.py` |
...
- **Con `DATABASE_URL` puesta** (ver §2): `python backend/db/migrate.py` aplica las pendientes;
  `python backend/db/migrate.py --check` solo lista el estado sin aplicar nada.
- **Sin `DATABASE_URL`** (situación de hoy): aplicar cada `.sql` nuevo de
  `backend/db/migrations/` a mano vía el MCP de Supabase o el SQL Editor del panel, y registrar el
  checksum en `schema_migrations` — ver el preámbulo de `SPECS-7A.md` §0.1 para el procedimiento
  exacto con `hashlib`.
